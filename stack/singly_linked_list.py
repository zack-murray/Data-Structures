...
class LinkedList:
    ...
    def remove_tail(self):
        ...
        if self.head is None:
            return
        # Instantiate current pointer
        current = self.head
        # Advance the current pointer to the tail node
        while current.get_next() and current.get_next() is not self.tail:
            current = current.get_next()
        # Instantiate tail value of list/stack
        value = self.tail.get_value()
        # Set self.tail to the current node
        self.tail = current
        # Make sure current pointer is set to new tail
